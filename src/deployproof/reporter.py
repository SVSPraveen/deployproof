...
def format_json_report(result: MutationResult, target_files: List[Path], secrets_result: Optional[SecretsScanResult]=None, symlink_result: Optional[SymlinkScanResult]=None, dependency_result: Optional[DependencyScanSummary]=None, mock_result: Optional[MockScanSummary]=None, control_flow_result: Optional[ControlFlowScanSummary]=None, strict_mocks: bool=False, strict_error_handling: bool=False, repo_root: Optional[Path]=None, threshold: float=80.0, version: Optional[str]=None) -> str:
    ...
    has_security_failure = bool(secrets_result and secrets_result.findings or (symlink_result and symlink_result.escape_findings) or (dependency_result and dependency_result.high_risk_count > 0) or strict_mocks_triggered or strict_error_triggered)
    is_passed = not result.collection_error and (not has_security_failure) and (not result.untested_files) and (result.mutation_score is not None and result.mutation_score >= threshold)
    ...
    for m in result.survived_mutants:
        ...
        surviving_mutants_data.append({'file': rel_f, 'line': m.line_number, 'description': m.description, 'original_code': m.original_line, 'mutated_code': m.mutated_line})
    ...
    for s in result.skipped_constructs:
        ...
        skipped_constructs_data.append({'file': rel_f, 'line': s.line_number, 'construct': s.construct_name, 'reason': getattr(s, 'description', getattr(s, 'reason', ''))})
    untested_files_data = []
    ...
    if secrets_result and secrets_result.findings:
        for f in secrets_result.findings:
            ...
            secrets_findings_data.append({'file': rel_f, 'line': f.line_number, 'rule': f.rule_name, 'severity': 'HIGH', 'redacted_value': f.redacted_value, 'snippet': f.snippet, 'message': f.description})
    ...
    if symlink_result and symlink_result.escape_findings:
        for f in symlink_result.escape_findings:
            ...
            symlink_escapes_data.append({'file': rel_f, 'target_raw': f.link_target_raw, 'resolved_target': str(f.resolved_target), 'target_exists': f.target_exists, 'severity': 'CRITICAL', 'message': f.description})
    dep_findings_data = []
    ...
    if dependency_result and dependency_result.findings:
        for f in dependency_result.findings:
            ...
            if f.status in ('HIGH_RISK', 'MEDIUM_RISK', 'UNKNOWN'):
                dep_findings_data.append({'package': f.package_name, 'import_name': f.import_name, 'file': rel_f, 'line': f.lineno, 'source_type': f.source_type, 'status': f.status, 'severity': 'HIGH' if f.status == 'HIGH_RISK' else 'MEDIUM' if f.status == 'MEDIUM_RISK' else 'UNKNOWN', 'age_days': f.age_days, 'first_release_date': f.first_release_date, 'message': f.details})
            elif f.status == 'UNSCANNED':
                unscanned_sources_data.append({'source': f.import_name, 'file': rel_f, 'line': f.lineno, 'source_type': f.source_type, 'reason': f.details})
    mock_findings_data = []
    if mock_result and mock_result.findings:
        for f in mock_result.findings:
            try:
                ...
                rel_f = f.file_path.as_posix()
            mock_findings_data.append({'file': rel_f, 'line': f.line_number, 'mock_type': f.mock_type, 'what': f.description, 'code': f.snippet, 'message': f'Newly added mock usage detected in diff: {f.description}'})
    ...
    if control_flow_result and control_flow_result.findings:
        for f in control_flow_result.findings:
            try:
                ...
                rel_f = f.file_path.as_posix()
            control_flow_findings_data.append({'file': rel_f, 'line': f.line_number, 'rule': f.rule_id, 'severity': f.severity, 'snippet': f.snippet, 'message': f.message})
    data: Dict[str, Any] = {'version': effective_version, 'status': status_str, 'summary': {'target_files_count': len(target_files), 'mutation_score': result.mutation_score, 'collection_error': result.collection_error, 'threshold': threshold, 'secrets_found': len(secrets_findings_data), 'symlink_escapes_found': len(symlink_escapes_data), 'dependency_findings': {'high_risk': dependency_result.high_risk_count if dependency_result else 0, 'medium_risk': dependency_result.medium_risk_count if dependency_result else 0, 'ok': dependency_result.ok_count if dependency_result else 0, 'unknown': dependency_result.unknown_count if dependency_result else 0, 'unscanned': len(unscanned_sources_data)}, 'mock_usages_found': len(mock_findings_data), 'control_flow_findings': len(control_flow_findings_data), 'strict_mocks_active': strict_mocks, 'strict_mocks_triggered': strict_mocks_triggered, 'strict_error_handling_active': strict_error_handling, 'strict_error_handling_triggered': strict_error_triggered}, 'scope': {'target_files': scope_files}, 'mutation_testing': {'score': result.mutation_score, 'collection_error': result.collection_error, 'threshold': threshold, 'total_mutants': result.total_mutants, 'killed_mutants': result.killed_mutants, 'survived_mutants_count': len(surviving_mutants_data), 'runner_errors_count': len(result.runner_errors), 'valid_mutants_count': result.killed_mutants + len(surviving_mutants_data), 'duration_seconds': result.duration_seconds, 'surviving_mutants': surviving_mutants_data, 'skipped_constructs': skipped_constructs_data, 'untested_files': untested_files_data}, 'secrets': {'clean': len(secrets_findings_data) == 0, 'files_scanned': secrets_result.files_scanned if secrets_result else len(target_files), 'findings': secrets_findings_data}, 'symlinks': {'clean': len(symlink_escapes_data) == 0, 'files_scanned': symlink_result.files_scanned if symlink_result else len(target_files), 'findings': symlink_escapes_data}, 'dependencies': {'clean': bool(not dep_findings_data), 'total_scanned': dependency_result.total_scanned if dependency_result else 0, 'findings': dep_findings_data, 'unscanned_sources': unscanned_sources_data}, 'mocks': {'clean': len(mock_findings_data) == 0, 'strict_gate_triggered': strict_mocks_triggered, 'findings': mock_findings_data}, 'control_flow': {'clean': len(control_flow_findings_data) == 0, 'strict_gate_triggered': strict_error_triggered, 'findings': control_flow_findings_data}}
    return json.dumps(data, indent=2)

def format_report(result: MutationResult, target_files: List[Path], secrets_result: Optional[SecretsScanResult]=None, symlink_result: Optional[SymlinkScanResult]=None, dependency_result: Optional[DependencyScanSummary]=None, mock_result: Optional[MockScanSummary]=None, control_flow_result: Optional[ControlFlowScanSummary]=None, strict_mocks: bool=False, strict_error_handling: bool=False, repo_root: Optional[Path]=None, threshold: float=80.0) -> str:
    ...
    if symlink_result and symlink_result.escape_findings:
        ...
        lines.append(f"  Clean: No sandbox-escape symlinks detected across {symlink_result.files_scanned} session files ({safe_count} safe in-repo symlink{('s' if safe_count != 1 else '')} verified).")
    else:
        ...
        for finding in dependency_result.findings:
            if finding.status in ('HIGH_RISK', 'MEDIUM_RISK'):
                ...
                if finding.status == 'HIGH_RISK':
                    ...
                    lines.append(f"        Classification: MEDIUM RISK (Registered {finding.age_days} day{('s' if finding.age_days != 1 else '')} ago, first published {finding.first_release_date})")
    ...
    if result.collection_error:
        ...
        if result.runner_errors:
            lines.append(f"  Score:  {result.mutation_score:.1f}% ({result.killed_mutants}/{valid_count} valid mutants killed, {len(result.runner_errors)} error{('s' if len(result.runner_errors) != 1 else '')} excluded)")
        else:
